...
BEGIN
    ...
    IF EXISTS(SELECT group_name FROM groups_ WHERE group_name = TRIM(in_group_name)) THEN
        SELECT 'DUPLICATE_GROUP' AS flag;
    ELSE
        INSERT INTO groups_ (group_name, group_image) VALUES (in_group_name, in_group_image);
        SELECT group_id INTO _group_id FROM groups_ WHERE group_name = in_group_name;
...
BEGIN
    ...
    FROM groups_ g JOIN groups_users gu
...
BEGIN
    ...
    IF EXISTS(SELECT group_id FROM groups_ WHERE group_name=in_group_name) THEN
        SELECT group_id INTO _group_id FROM groups_ WHERE group_name=in_group_name;
...
sp: BEGIN
    ...
    FROM (
        ...
        FROM (
            ...
            JOIN groups_ g ON b.group_id = g.group_id
...
BEGIN
    SELECT b.bill_id, 
            ...
            b.bill_created_at
    FROM bills b
    LEFT JOIN groups_ g ON b.group_id = g.group_id
